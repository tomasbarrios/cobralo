...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create blues-stack-1dcb
  fly apps create blues-stack-1dcb-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app blues-stack-1dcb
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app blues-stack-1dcb-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'blues-stack-1dcb-staging' does not match app name in config file 'blues-stack-1dcb'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name blues-stack-1dcb-db
  fly postgres attach --app blues-stack-1dcb blues-stack-1dcb-db

  fly postgres create --name blues-stack-1dcb-staging-db
  fly postgres attach --app blues-stack-1dcb-staging blues-stack-1dcb-staging-db
  ```
