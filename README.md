...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create cobralo
  fly apps create cobralo-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app cobralo
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app cobralo-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'cobralo-staging' does not match app name in config file 'cobralo'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name cobralo-db
  fly postgres attach --app cobralo cobralo-db

  fly postgres create --name cobralo-staging-db
  fly postgres attach --app cobralo-staging cobralo-staging-db
  ```
